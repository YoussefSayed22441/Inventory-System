...
import { MagneticButton, EnergyScanFrame, HoldToConfirm } from '../components/ui/MotionEffects';
...
function GeneralPanel({ settings, onChange }) {
  return (
    <motion.div key="general" className="settings-content"
      ...
      <EnergyScanFrame variant="panel">
        <div className="glass-panel settings-panel">
          ...
          <div className="settings-row">
            ...
            <select className="settings-select" value={settings.timezone}
 ...
 *  SECURITY PANEL
 *  ──
 *  Crimson Override: when the user starts holding the Deactivate button,
 *  `crimsonMode` flips true, overriding every CSS orange variable inside the
 *  danger-zone wrapper to harsh crimson (#DC143C).  The override reverts if
 *  the user releases before 2 s.  On full confirm it stays crimson.
 * ─────────────────────────────────────────────────────────────────── */
function SecurityPanel({ settings, onChange }) {
  const [crimsonMode, setCrimsonMode] = useState(false);
  const [deactivated, setDeactivated] = useState(false);

  /* CSS variable overrides applied to the danger-zone wrapper */
  const crimsonVars = crimsonMode ? {
    '--neon-orange':         '#DC143C',
    '--neon-orange-hover':   '#A50E2D',
    '--neon-orange-dim':     'rgba(220,20,60,0.15)',
    '--neon-orange-glow':    '0 0 24px rgba(220,20,60,0.5)',
    '--glass-border-orange': 'rgba(220,20,60,0.4)',
    '--glow-orange':         '0 0 16px rgba(220,20,60,0.35)',
    '--glow-orange-strong':  '0 0 30px rgba(220,20,60,0.7)',
  } : {};

  return (
    <motion.div key="security" className="settings-content"
      variants={panelVariants} initial="hidden" animate="visible" exit="exit">
      ...
      <EnergyScanFrame variant="panel">
        <div className="glass-panel settings-panel">
          ...
          <div className="settings-row">
            ...
            <select className="settings-select" value={settings.sessionDuration}
      ...
      {/* ── DANGER ZONE ── crimsonVars override applied here ─────── */}
      <motion.div
        className={`glass-card danger-zone${crimsonMode ? ' crimson-active' : ''}`}
        style={crimsonVars}
        animate={crimsonMode ? { boxShadow: 'var(--glass-shadow), 0 0 40px rgba(220,20,60,0.35)' } : {}}
        transition={{ duration: 0.5 }}
      >
        <div className="danger-zone__header">
          <motion.div
            className="danger-zone__icon"
            animate={crimsonMode ? { rotate: [0, -8, 8, -8, 0] } : { rotate: 0 }}
          ...
          <div>
            <div className="danger-zone__title">
              {crimsonMode ? '⚠ CRIMSON OVERRIDE ACTIVE' : 'Danger Zone'}
            </div>
            <div className="danger-zone__subtitle">
              {crimsonMode
                ? 'Deactivation sequence armed — hold to confirm'
                : 'Irreversible and destructive actions'}
            </div>
          </div>
        </div>

        {/* Export data — magnetic */}
        <div className="danger-action">
          <div className="danger-action__info">
            <h4>Download My Data</h4>
            <p>Export a full archive of your account data as a .zip file</p>
          </div>
          <MagneticButton className="btn-danger" strength={0.2}>
            <Download size={14} /> Export Data
          </MagneticButton>
        </div>

        {/* Reset preferences — magnetic */}
        <div className="danger-action">
          <div className="danger-action__info">
        ...
        {/* ── DEACTIVATE — Hold-to-Confirm + Crimson Override trigger ── */}
        <div className="danger-action">
          <div className="danger-action__info">
            <h4>Deactivate Account</h4>
            <p>
              {deactivated
                ? 'Account has been deactivated. Contact an admin to reactivate.'
                : 'Hold the button for 2 s to temporarily disable your account'}
            </p>
          </div>
          {!deactivated ? (
            <HoldToConfirm
              ...
              icon={<UserX size={14} />}
              duration={2000}
              onHoldStart={() => setCrimsonMode(true)}
              onHoldEnd={() => setCrimsonMode(false)}
              onConfirm={() => { setDeactivated(true); setCrimsonMode(true); }}
            />
          ) : (
            <span className="danger-confirmed-badge">
              ✓ Deactivated
            </span>
          )}
        </div>

        {/* Delete forever — magnetic */}
        <div className="danger-action">
          <div className="danger-action__info">
...
function NotificationsPanel({ settings, onChange }) {
  return (
    <motion.div key="notifications" className="settings-content"
      variants={panelVariants} initial="hidden" animate="visible" exit="exit">
        ...
        <div className="glass-panel settings-panel">
          ...
          <div className="settings-row">
            <div className="settings-row__info">
              ...
              <div className="settings-row__desc">Immediate notification for login anomalies</div>
            </div>
      ...
      <EnergyScanFrame variant="panel">
        <div className="glass-panel settings-panel">
          ...
          <div className="settings-row">
            ...
            <select className="settings-select" value={settings.notifFreq}
...
export default function Settings() {
  ...
  const [settings, setSettings]  = useState({
    darkMode: true,  compactView: false, animations: true,  language: 'en',
    autoSave: true,  telemetry: false,   timezone: 'utc+2',
    twoFA: true,     biometric: false,   loginAlerts: true, sessionDuration: '8h',
    profileVisible: true, activityStatus: true, auditLog: true,
    emailOrders: true, emailStock: true, emailReport: false, emailSecurity: true,
    pushDesktop: true, soundAlerts: false, mentions: true, notifFreq: 'realtime',
  });
  ...
  return (
    <motion.div className="settings-page" variants={pageVariants} initial="hidden" animate="visible">
      ...
      <motion.div className="settings-layout" variants={childVariants}>
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'general'       && <GeneralPanel       key="general"       settings={settings} onChange={handleChange} />}
          {activeTab === 'security'      && <SecurityPanel      key="security"      settings={settings} onChange={handleChange} />}
          {activeTab === 'notifications' && <NotificationsPanel key="notifications" settings={settings} onChange={handleChange} />}
        </AnimatePresence>
      </motion.div>
    </motion.div>
  );
